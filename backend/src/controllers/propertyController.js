...
const getProperties = async(req,res)=>{
    try{
      ...
      // total number of properties that match the filters/search
      // (without pagination) - the frontend needs it to know the last page
      const countFeatures = new APIFeatures(Property.find(),req.query)
      .filter()
      .search();
      const allProperties = await countFeatures.query.countDocuments();
...
// DELETE MY PROPERTY - an owner removes his own house
// Rules:
// 1. Only the owner can delete (checked with the token, not the body).
// 2. If a guest still has an upcoming/ongoing booking, it cannot be deleted.
// 3. Photos are removed from ImageKit, old (finished) bookings are cleaned up.
  ...
  try {
    ...
    if (!property.userId || property.userId.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        status: "fail",
        message: "You can delete only your own accommodation",
      });
    }

    // block delete when a guest has an upcoming or ongoing booking
    ...
    if (activeBookings > 0) {
      return res.status(400).json({
        status: "fail",
        message:
          "This accommodation has upcoming bookings, so it cannot be deleted right now",
      });
    }

    // remove photos from ImageKit (a failure here must not stop the delete)
    ...
    // remove old finished bookings of this property, then the property
